"""
Database configuration and ORM models — Dental Camp EMR (PostgreSQL + SQLAlchemy 2.0)
Includes all shared tables (patients, visits, stock) plus dental_visits.
"""
...
class DentalVisit(Base):
    ...
    history_complaint     = Column(Text)
    la_experience         = Column(String(5))
    scaling               = Column(String(5))
    filling_rct           = Column(String(5))
    extraction            = Column(String(5))
    prosthesis            = Column(String(5))
    smoking               = Column(String(5))
    gutkha                = Column(String(5))
    naswar                = Column(String(5))
    pan                   = Column(String(5))
    mauva                 = Column(String(5))
    alcohol               = Column(String(5))
    brushing_type         = Column(String(50))
    brushing_freq         = Column(String(50))
    brushing_timing       = Column(String(50))
    medical_history_notes = Column(Text)
    dentition_status      = Column(Text)   # stored as JSON string
    provisional_diagnosis = Column(Text)
    medicines             = Column(Text)
    dispensed             = Column(String(10), default="No")
    pre_op_image          = Column(String(255))
    post_op_image         = Column(String(255))
...
def init_db() -> None:
    ...
    try:
        ...
        pass  # pg_trgm is optional; requires superuser
